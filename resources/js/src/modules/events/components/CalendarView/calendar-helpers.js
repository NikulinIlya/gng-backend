...
export const getCalendarData = (date = new Date()) => {
  ...
  const weeksInMonth = Math.ceil((daysInMonth + monthStartsOn) / DAYS_IN_WEEK);
  let day = 1;
  ...
  const monthData = Array.from({ length: weeksInMonth }).reduce(
    (acc, _, weekIndex) => {
      const week = Array.from({ length: DAYS_IN_WEEK }).map((_, dayIndex) => {
        if (!weekIndex && dayIndex < monthStartsOn) {
          return new Date(year, month, (monthStartsOn - dayIndex - 1) * -1);
        }
